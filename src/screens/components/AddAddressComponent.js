...
import {onAddAddress} from '../../redux/actions/UserAction';
import * as yup from 'yup';

const reviewSchema = yup.object({
  addressType: yup.string().required('Required Field').min(5),
    ...
    <View behavior="position" style={styles.container}>
      ...
      <KeyboardAwareScrollView>
        <Formik
          ...
          onSubmit={(values) => {
            const check = item.some(
              (address) => address.addressType === values.addressType,
            );
            if (!check) {
              dispatch(onAddAddress(values));
              onClose();
            } else {
              alert(`${values.addressType} Address already exists`);
            }
          }}>
          {(props) => {
            return (
